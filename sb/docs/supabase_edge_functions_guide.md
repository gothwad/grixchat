# Supabase Edge Functions Guide (GrixChat)

This guide contains the exact source codes for GrixChat's core **Supabase Edge Functions**. Since you can create and deploy functions directly using the Supabase Web Dashboard Editor, you can copy the code below and paste it into the online editor's `index.ts` file.

---

## 1. `livekit-token` Edge Function
This function issues dynamic JSON Web Tokens (JWT) for voice and video calling participants using LiveKit.
...
This function dispatches secure bulk push notifications to recipients via **Firebase Cloud Messaging (FCM)**.

### Configuration / Secret Required:
- **`FIREBASE_SERVICE_ACCOUNT`**: The full JSON content of your downloaded Firebase Service Account Private Key file (`.json`). Paste the entire file content here.
